...
app.controller('mainCtrl', function($scope, itunesService){
  ...
  //Also note that that method should be retuning a promise, so you could use .then in this function.
    ...
    $scope.getSongData = function(){
      itunesService.getSongs($scope.artist).then(function(arrayFromItunes){

        $scope.songData = mapTheSongDataForNgGrid(arrayFromItunes);
      });
    };

    function mapTheSongDataForNgGrid(songs){
  ...
  */
  ...
  //and then loop through the iTunes data pushing into your new array objects that look like the above data.
